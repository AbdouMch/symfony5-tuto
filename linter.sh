...
if [[ -n "$CHANGED_PHP_FILES" ]]; then
    ...
    echo ""
    echo "- PhpStan"
    if docker exec -w /srv/app symfony_5 ./vendor/bin/phpstan --xdebug analyse -- "${CHANGED_PHP_FILES_ARR[@]}"; then
        echo ""
    else
        RC=1
    fi
    ...
    echo ""
    echo "- Container Lint"
    if docker exec -w /srv/app symfony_5 php bin/console lint:container; then
        echo ""
    else
        RC=1
    fi
...
if [[ -n "$CHANGED_TWIG_FILES" ]]; then
    ...
    echo ""
    echo "- TWIG Lint"
    if docker exec -w /srv/app symfony_5 php bin/console lint:twig "${CHANGED_TWIG_FILES_ARR[@]}" --show-deprecations; then
        echo ""
    else
        RC=1
    fi
